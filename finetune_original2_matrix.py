...
def finetune_classify(liz_x,y, model, state_in, save_it, linear = False, flatten = True, n_query = 15, ds= False, pretrained_dataset='miniImageNet', freeze_backbone = False, n_way = 5, n_support = 5): 
    ...
    score2 = torch.nn.functional.softmax(score1, dim = 1).detach() + torch.nn.functional.softmax(score_b, dim = 1).detach()
    return score, score2
...
if __name__=='__main__':
  ...
  if params.method != "all":
    for idx, (elem) in enumerate(novel_loader):
      ...
      cmat1 = confusion_matrix(y_query, topk_ind, normalize = 'all')
      cmat2 = confusion_matrix(y_query, topk_ind2, normalize = 'all')

      print(cmat1)
      print(check_symmetric(cmat1))
      print(cmat2)
      print(check_symmetric(cmat2))

      if check_symmetric(cmat1):
        symm_all1.append(1)
      else:
        symm_all1.append(0)

      if check_symmetric(cmat2):
        symm_all2.append(1)
      else:
        symm_all2.append(0)
  ...
  symm_all1 = np.asarray(symm_all1)
  std_s1  = np.std(symm_all1)
