...
'''
Below we define all the self-supervised learning techniques

'''


def ss_lab_cb(score_vec, n_way, n_query, num_lab = 2):
    max_val_tup = torch.max(score_vec, 1)
...
def ss_top(score_vec, n_way, n_query, num_lab = 2):
    max_val_tup = torch.max(score_vec, 1)
    argmax_val = max_val_tup[1]
    max_val = max_val_tup[0]
    max_val_idx = [(i, val) for i,val in enumerate(max_val)]
