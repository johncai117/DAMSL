...
class GnnNet_Analogy(MetaTemplate):
  ...
  def __init__(self, model_func,  n_way, n_support, params):
    ...
    # metric function
    self.fc = nn.Sequential(nn.Linear(self.feat_dim, 32), nn.BatchNorm1d(32, track_running_stats=False)) if not self.maml else nn.Sequential(backbone.Linear_fw(self.feat_dim, 32), backbone.BatchNorm1d_fw(32, track_running_stats=False))
    ...
    self.fc2 = copy.deepcopy(baseline_model.classifier)
    del baseline_model ##clear memory
    self.gnn = GNN_nl(32 + 64 + self.n_way, 32, self.n_way)
    self.method = 'GnnNet'
  ...
  def train_loop_finetune(self, epoch, train_loader, optimizer ):
        ...
        for i, (x,y ) in enumerate(train_loader):
            ...
            loss = self.set_forward_loss_finetune( x,y )
            loss.backward()
            optimizer.step()
            self.MAML_update()
  ...
  def set_forward_finetune(self,x,y,is_feature=False):
    x = x.cuda()
    ...
    #print(final.shape)
    #print(final_b.shape)
    z_b = F.dropout(self.fc2(final_b.view(-1, *final_b.size()[2:])))
    z_b = z_b.view(self.n_way, -1, z_b.size(1)).detach()
