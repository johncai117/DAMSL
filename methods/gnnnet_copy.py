...
class GnnNet(MetaTemplate):
  ...
  def train_loop50(self, epoch, train_loader, optimizer ):
        print_freq = 10

        avg_loss=0
        for i, (x,_ ) in enumerate(train_loader):
            self.n_query = x.size(1) - 50           
            if self.change_way:
                self.n_way  = x.size(0)
            optimizer.zero_grad()
  ...
  def train_loop_finetune50(self, epoch, train_loader, optimizer ):
        print_freq = 10
  ...
  def set_forward_finetune(self,x,is_feature=False):
    ...
    # get feature using encoder
    batch_size = 4
    true_n_support = self.n_support * 2
    support_size = self.n_way * true_n_support
    ...
    y_a_i = Variable( torch.from_numpy( np.repeat(range( self.n_way ), true_n_support ) )).cuda() # (25,)
    ...
    x_b_i = x_var[:, true_n_support:,:,:,:].contiguous().view( self.n_way* self.n_query,   *x.size()[2:]) 
    x_a_i = x_var[:,:true_n_support,:,:,:].contiguous().view( self.n_way* true_n_support, *x.size()[2:]) # (25, 3, 224, 224)
    ...
    output_support = self.feature(x_a_i.cuda()).view(self.n_way, true_n_support, -1)
    ...
    #print(x.size(1))
    #print(x.shape)
    assert(final.size(1) == (true_n_support) + 16) ##16 query samples in each batch
